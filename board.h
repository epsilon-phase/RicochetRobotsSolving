...
public:
  ...
  bool
  getHorizontalWall(double r, int c) const;
  bool
  getVerticalWall(int r, double c) const;
  ...
  int
  whichRobot(char a) const;
  // given a robot's id, lookup the name
  char
  getRobot(int i) const
  {
    return robots[i];
  }
  // get the robots current position
  Position
  getRobotPosition(int i) const
  {
  ...
  void
  addHorizontalWall(double r, int c);
  void
  addVerticalWall(int r, double c);

  // MODIFIERS related robot position
  // initial placement of a new robot
  void
  placeRobot(const Position &p, char a);
